...
if [ ! -d "${CAPTURE_DIR}" ]; then
  echo "Specified capture directory ${CAPTURE_DIR} does not exist!"
  exit 2
fi

echorun docker run -i --rm -v ${CAPTURE_DIR}:/source \
       ${IMAGE} replay \
       --bootstrap-servers ${BOOTSTRAP_SERVERS} --topic ${TOPIC} \
       --source-directory /source --source-format "${CAPTURE_FORMAT}"