...
public class DockerTestKafkaBackupManagerDifferentApps extends AbstractKafkaBackupManagerTests {
    ...
    public void givenKafkaBackupManagersForDifferentApps_whenTransitioningStates_thenNotSynced(
        ...
        try (KafkaSink<UUID, BackupTransition> sink = createSink()) {
            ...
            // When
            try (BackupManager primary = KafkaPrimaryBackupManager.builder().application("app-a").sink(sink).build()) {
                // Then
                verifySecondaryManager("app-b", source, transitions, primary, BackupManagerState.STARTING);
