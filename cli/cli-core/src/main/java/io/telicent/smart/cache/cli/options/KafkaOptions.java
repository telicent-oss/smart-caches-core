...
public class KafkaOptions {
    ...
    /**
     * Environment variable used to specify Kafka output dead letter topic, where events with processing errors will be written.
     */
    ...
    @Option(name = { "-t", "--topic" }, title = "KafkaTopic",
            description = "Provides the name of the Kafka topic(s) to read events from.  May be specified multiple times to supply multiple topics, when multiple topics are specified then they must all contain events that can be deserialized by this command otherwise errors will occur!")
    @RequiredForSource(sourceName = "Kafka", unlessEnvironment = { TOPIC, INPUT_TOPIC })
    public Set<String> topics =
            Configurator.get(new String[] { TOPIC, INPUT_TOPIC }, t -> {
     ...
     * Kafka bootstrap servers
     */
    @Option(name = { "-dlq", "--dlq-topic" }, title = "KafkaDlqTopic",
            description = "Provides the name of a Kafka dead letter topic, where events with processing errors will be written.")
    public String dlqTopic = Configurator.get(DLQ_TOPIC);

    /**
     * Kafka consumer group
     */
    @Option(name = { "-g", "--group" }, title = "KafkaConsumerGroup",
            description = "Provides the name of the Kafka Consumer Group to use.  If not set defaults to the name of the command being invoked, or smart-cache if that is unknown.")
    ...
    public Properties getAdditionalProperties() {
        ...
        if (StringUtils.isNotBlank(this.username) && StringUtils.isNotBlank(this.password)) {
            ...
            properties.put(SaslConfigs.SASL_JAAS_CONFIG, KafkaSecurity.plainLogin(this.username, this.password));
        }
        return properties;
    }
     ...
     * Kafka read policy
     */
    @Option(name = "--read-policy", title = "KafkaReadPolicy",
            description = "Specifies how events should be read from Kafka.  Defaults to EARLIEST i.e. automatically read from the earliest available events that have yet to be consumed by the specified Consumer Group.")
