...
public class DockerTestKafkaBackupTracker extends AbstractKafkaBackupTrackerTests {
    ...
    public void givenKafkaBackupTrackers_whenTransitioningStates_thenStateTrackersAreSynced(
        ...
        try (KafkaSink<UUID, BackupTransition> sink = createSink()) {
            ...
            try (BackupTracker primary = KafkaPrimaryBackupTracker.builder().application("test").sink(sink).build()) {
                verifySecondaryTracker("test", source, transitions, primary, expectedFinalState);
            }
        }
    }
